...
async def apply_quota_movement(
    ...
    # No try/except-and-fabricate-success here on purpose: a masked exception was
    # previously indistinguishable from a real "ok" result (callers only branch on
    # status == "insufficient"), which silently disabled quota enforcement on any
    # transient DB error, and the blanket db.rollback() inside the old fallback could
    # discard unrelated work already flushed earlier in the same caller transaction
    # (e.g. the just-inserted chat user message). Let failures propagate — the caller
    # already runs inside get_db()'s transaction and will roll back cleanly, and the
    # client gets an honest 500 instead of a fabricated "applied".
    ...
    if not row:
        raise RuntimeError(
            "quota_apply_movement returned no row — this should be unreachable."
        )

    # NOTE: the RPC's RETURNS TABLE columns are audio_seconds_remaining /
    # agent_queries_remaining (see migrations/003_functions_and_triggers.sql), not
    # audio_remaining/agent_remaining. Reading the wrong keys here used to raise a
    # KeyError on every single call (success or dedup-hit alike), which is what made
    # the old blanket except-and-fabricate-success fallback fire unconditionally —
    # quota enforcement was never actually being exercised.
    status_val = row["status"]
    return QuotaMovementResult(
        status="already_applied" if status_val == "already_processed" else status_val,
        audio_remaining=int(row["audio_seconds_remaining"]),
        agent_remaining=int(row["agent_queries_remaining"]),
    )
    ...
    for meeting in stuck_meetings:
        ...
        try:
            ...
            res_reserve = await db.execute(
                text(
                    "SELECT delta_audio_seconds FROM public.quota_movements WHERE dedup_key = :key LIMIT 1"
                ),
            ...
            res_settle = await db.execute(
                text(
                    "SELECT id FROM public.quota_movements WHERE dedup_key = :key LIMIT 1"
                ),
    ...
    for wallet in wallets:
        ...
        try:
            ...
            res_ledger = await db.execute(
                text(
                    "SELECT delta_audio_seconds, delta_agent_queries FROM public.quota_movements WHERE user_id = :user_id"
                ),
                {"user_id": user_id},
            )
            rows = res_ledger.mappings().all()

            ledger_audio = sum(int(r["delta_audio_seconds"]) for r in rows)
            ledger_queries = sum(int(r["delta_agent_queries"]) for r in rows)

            wallet_audio = int(wallet["audio_seconds_remaining"])
            ...
            has_drift = (audio_drift != 0) or (query_drift != 0)
            ...
            if result.status != "already_applied":
                ...
                logger.info(
                    ...
                    f"audio={audio_drift:+d}s, queries={query_drift:+d}"
                )
