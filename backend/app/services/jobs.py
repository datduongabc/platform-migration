...
async def _download_audio(audio_path: str) -> bytes:
    ...
    candidates = [
        ...
        os.path.join("uploads", audio_path),
        os.path.join("uploads", audio_path.replace("recordings/", "")),
    ]
    for cand in candidates:
        if os.path.exists(cand):

            def _read_local(p=cand):
                with open(p, "rb") as f:
                    return f.read()
...
def is_terminal_error(err: Exception) -> bool:
    """
    Terminal errors should NOT be retried — they indicate bad configuration or data
    issues, or a provider's final (non-recoverable) verdict on this specific job.
    Everything else (network blips, exhausted-but-recoverable key pools, DB hiccups)
    is transient and gets retried with backoff.
    """
    err_msg = str(err)
    if "401" in err_msg or "API_KEY_INVALID" in err_msg:
        return True
    # A permanent Speechmatics rejection/deletion for this specific job (raised in
    # _run_transcribe_poll_step) is a final verdict, not a flake — retrying just
    # wastes up to max_attempts rounds of exponential backoff before failing anyway.
    if "Speechmatics job" in err_msg and (
        "rejected:" in err_msg or "unexpectedly in status" in err_msg
    ):
        return True
    if isinstance(
...
async def _run_start_step(
    ...
    # 1. Update meeting status to processing (atomic claim). Checking rowcount here
    # (not just re-selecting by id, which always finds the row regardless of whether
    # the claim actually matched) is what makes this a real guard — without it,
    # re-running this step on an already 'processing' or 'done' meeting silently
    # re-does the whole pipeline (re-downloads audio, re-reserves quota, resubmits
    # to Speechmatics) instead of being the safe no-op it's supposed to be.
    ...
    if claimed:
        ...
        # Not claimed: either a restart-continuation of THIS job (meeting already
        # 'processing' from a prior attempt, e.g. after a worker crash or the
        # stuck-job sweeper requeuing it) — resume it — or genuinely not claimable
        # (e.g. already 'done'), in which case this is a safe no-op.
        res = await db.execute(
            text(
    ...
    try:
            ...
            if estimate_seconds > 0:
                reserve = await apply_quota_movement(
                    ...
                    QuotaMovementParams(
                        ...
                        # Must be False: this is the hard pre-flight gate. With True,
                        # quota_apply_movement's insufficient-balance branch is
                        # unreachable and generation can never be blocked.
                ...
                if reserve.status == "insufficient":
                    ...
                    await db.execute(
                        text("""
                            INSERT INTO public.activity_log (id, user_id, event_type, meeting_id, metadata, created_at)
        ...
        if not tmp_audio_path.lower().endswith(".mp3"):
            ...
            target_mp3 = os.path.join(tmp_transcode_dir, "audio.mp3")
            try:
                res_path = await transcode_to_mp3(tmp_audio_path, target_mp3)
                if res_path and os.path.exists(res_path):
                    job_audio_path = res_path
                    logger.info(
                        f"[jobs/start] {meeting_id}: transcoded to MP3: {job_audio_path}"
                    )
                else:
                    job_audio_path = tmp_audio_path
...
async def _run_transcribe_poll_step(
    ...
    if status == "done":
        ...
        # Settle quota. audio_seconds_remaining is an integer column (whole
        # seconds), so the real duration from Speechmatics must be rounded up
        # before computing the delta — subtracting the raw float here previously
        # produced fractional deltas like 0.8600000000000136 that only looked
        # sane by accident when the column type still tolerated floats.
        user_id = payload.get("user_id")
        estimate_seconds = payload.get("estimate_seconds") or 0
        real_seconds = math.ceil(audio_seconds)
        if user_id and estimate_seconds > 0:
            await apply_quota_movement(
                db,
                QuotaMovementParams(
                    user_id=user_id,
                    delta_audio_seconds=estimate_seconds - real_seconds,
                    delta_agent_queries=0,
                    reason="generate",
        ...
        # Insert segments in one batched round-trip instead of one await per row —
        # a long meeting can have hundreds of segments, and awaiting each INSERT
        # individually serialized the whole insert on network latency to Postgres.
...
async def _run_analyse_step(
    ...
    # Fetch meeting metadata (started_at, user_id for AIP-06 model resolution)
    res_mtg = await db.execute(
        text("SELECT started_at, user_id FROM public.meetings WHERE id = :id"),
        {"id": meeting_id},
    )
    ...
    owner_id = str(meeting["user_id"]) if meeting and meeting["user_id"] else None

    _, resolved_model = await resolve_generation_model(db, user_id=owner_id)

    # Call Gemini analysis. On failure/empty-result this now raises (see
    # gemini.py) instead of silently fabricating summary/notes from raw
    # transcript text — a real analysis failure surfaces as a failed job with
    # the normal retry/terminal-fail handling, rather than a "successful" but
    # low-quality result.
    logger.info(
        f"[jobs/analyse] {meeting_id}: calling Gemini ({resolved_model}) with {len(seg_rows)} segments"
    )
    analysis = await analyze_transcript(
        segments, meeting_date=started_at, model=resolved_model
    )
    logger.info(
    ...
    # Insert todos — batched in one round-trip rather than one await per todo.
    if analysis.todos:
        await db.execute(
            text("""
            ...
            [
                {
                    ...
                    "due_date": t.due_date,
                    "source_segment_id": (
                        segment_id_by_index.get(t.source_segment_index)
    ...
    # Insert calendar suggestions — same batching rationale as todos above.
    if analysis.calendar_suggestions:
        await db.execute(
            text("""
            ...
            [
                {
                    ...
                    "source_segment_id": (
                        segment_id_by_index.get(c.source_segment_index)
...
async def _run_embed_step(
    ...
    if chunks:
        ...
        # Insert chunks — batched in one round-trip rather than one await per chunk.
        await db.execute(
            text("""
                INSERT INTO public.transcript_chunks
                (id, meeting_id, chunk_index, content, embedding, start_ms, end_ms, token_count)
                VALUES (:id, :meeting_id, :chunk_index, :content, CAST(:embedding AS vector), :start_ms, :end_ms, :token_count)
            """),
            [
                {
                    "id": str(uuid.uuid4()),
