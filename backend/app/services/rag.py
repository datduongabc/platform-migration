...
async def retrieve_context(
    ...
    folder_id: Optional[UUID | str] = None,
) -> List[Dict[str, Any]]:
    """
    ...
    sql = text("""
        SELECT * FROM public.match_transcript_chunks(
            query_embedding := CAST(:query_embedding AS vector),
    ...
    result = await db.execute(
        sql,
        {
            ...
            # Always scope to the caller — without this, global-scope chat
            # (meeting_id is None) searched every user's transcripts.
            "filter_user_id": str(user_id) if user_id else None,
            "filter_folder_id": str(folder_id) if folder_id else None,
        },
    )
